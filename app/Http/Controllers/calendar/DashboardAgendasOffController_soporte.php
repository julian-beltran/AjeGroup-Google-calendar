...
class DashboardAgendasOffController extends Controller
{
    ...
    public function index(){
        ...
        // try{
            ...
            if(isset($_SESSION['access_token']) && $_SESSION['access_token']){
                ...
                // PARTE 4: ------------------------------------------------------------------------------------------
                // Muestra los usuarios para espacio OTO tengan o no tengan y si su fecha meet es mayor a la actual
                $data_de_usuarios = DB::table('users')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'individual')->limit(1);
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "individual" limit 1)')
                ...
                $usuarios_con_agendas = DB::table('users')
                    ...
                    ->whereNotExists(function ($query) use ($id_areas) {
                ...
                $usuarios_de_area_individual = $data_de_usuarios->unionAll($usuarios_con_agendas)->get();

                $data_de_usuarios_country = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'country');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "country")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_country = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_country = $data_de_usuarios_country->unionAll($usuarios_con_agendas_country)->get();

                $data_de_usuarios_primario = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'primario');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "primario")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_primario = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_primario = $data_de_usuarios_primario->unionAll($usuarios_con_agendas_primario)->get();

                $data_de_usuarios_compras = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'compras');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "compras")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_compras = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_compras = $data_de_usuarios_compras->unionAll($usuarios_con_agendas_compras)->get();

                $data_de_usuarios_merco = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'merco');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "merco")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_merco = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_merco = $data_de_usuarios_merco->unionAll($usuarios_con_agendas_merco)->get();

                $data_de_usuarios_indicadores = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'indicadores');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "indicadores")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_indicadores = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_indicadores = $data_de_usuarios_indicadores->unionAll($usuarios_con_agendas_indicadores)->get();

                $data_de_usuarios_sostenibilidad = DB::table('users')
                    ->leftJoin('agenda_invitados', 'users.id', '=', 'agenda_invitados.id_user')
                    ->leftJoin('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                    ->leftJoin('espacios', 'agendas.id_espacio', '=', 'espacios.id')
                    ->leftJoin('areas', 'areas.id', '=', 'agendas.id_area')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'sostenibilidad');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "sostenibilidad")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas_sostenibilidad = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($id_areas) {
                        $query->select(DB::raw(1))
                ...
                $usuarios_de_area_sostenibilidad = $data_de_usuarios_sostenibilidad->unionAll($usuarios_con_agendas_sostenibilidad)->get();
    ...
    public function ver_datos_para_agendar_evento($userId, $espacioId, $areaId)
    {
        // Log::info('LLegó al controller:');
        try{
            // Log::info('Acceso al Try-Catch.');
            if (request()->ajax()) {
                $user       = User::findOrFail($userId);
                $espacio    = Espacio::findOrFail($espacioId);
                $area       = Area::findOrFail($areaId);

                /*Log::info('USUARIO OBTENIDO: '.json_encode($user));
                    Log::info('ESPACIO OBTENIDO: '.json_encode($espacio));
                    Log::info('AREA OBTENIDA: '.json_encode($area));
                */
    ...
    public function guardar_agenda_individual(Request $request)
    {
        try {
            ...
            Log::info('Hora obtenida: '.json_encode($fecha_hora_meet));
            Log::info('Hora agregada: '.json_encode($fecha_hora_termino));
            ...
            Log::info('agenda individual validada: '.json_encode($agendas_validate));
            ...
            if (isset($_SESSION['access_token']) && $_SESSION['access_token']) {
                ...
                if ($results) {
                    ...
                    $agenda->save();
                    Log::info('Guardado usuario: ' . json_encode($agenda));
                    // Asignar el usuario a la agenda principal
                    $agenda->users()->attach($id_user_invitado);
                    Log::info('Usuario vinculado a la agenda: ' . json_encode($agenda));
                    ...
                    Log::info('Datos preparados para enviar el correo electrónico de la agenda => ' . json_encode($dataMail));

                    // Envío del correo electrónico
                    Mail::to($user_email)->send(new SendAgendaMail($dataMail));

                    Log::info("Evento creado en base de datos: ****[" . json_encode($agenda) . "]****");
                } else {
    ...
    {
        Log::info('Llegó al controller para obtener data para subir agenda.');
        try {
    ...
    {
        Log::info('Llegó al controller para obtener data para cerrar agenda con evidencia opcional.');
        try {
            ...
            if (request()->ajax()) {
                ...
                $data_de_usuarios = DB::table('users')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'pares');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "pares")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

                $usuarios_con_agendas = DB::table('users')
                    ->leftJoin('area_user', 'users.id', '=', 'area_user.id_user')
                    ->leftJoin('areas', 'area_user.id_area', '=', 'areas.id')
                    ->whereNotExists(function ($query) use ($areaId) {
                        $query->select(DB::raw(1))
                            ->from('agenda_invitados')
                            ->join('agendas', 'agenda_invitados.id_agenda', '=', 'agendas.id')
                            ->join('areas', 'agendas.id_area', '=', 'areas.id')
            ...
            if (request()->ajax()) {
                ...
                $data_de_usuarios = DB::table('users')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'max 10');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "max 10")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));
    ...
    {
        Log::info('LLegó al controller de data grupal para seleccionar usuarios con seleccion:');
        try{
            Log::info('Acceso al Try-Catch de select usuarios.');
            if (request()->ajax()) {
                $espacio    = Espacio::findOrFail($espacioId);
                ...
                $data_de_usuarios = DB::table('users')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'ranking');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "ranking")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));
            ...
            if (request()->ajax()) {
                ...
                $data_de_usuarios = DB::table('users')
                    ...
                    ->where('espacios.id', function ($query) {
                        $query->select('id')
                            ->from('espacios')
                            ->where('tipo_reunion', 'retroalimentacion');
                    })
                    ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                    ->groupBy('users.id', 'name', 'email')
                    ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "retroalimentacion")')
                    ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));
    ...
    {
        Log::info('LLegó al controller de data grupal para COUNTRY:');
        try{
            Log::info('Acceso al Try-Catch de country.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);

                $users = User::whereIn('id', $users_ids)->get();
                Log::info('Usuarios encontrados: '.json_encode($users));
                $espacio    = Espacio::findOrFail($espacioId);
                Log::info('Espacio encontrado para select usuarios: '.json_encode($espacio));
                $area       = Area::findOrFail($areaId);
                Log::info('Area encontrada para user select: '.json_encode($area));
    ...
    {
        Log::info('LLegó al controller de data grupal para PRIMARIO:');
        try{
            Log::info('Acceso al Try-Catch de primario.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);
    ...
    {
        Log::info('LLegó al controller de data grupal para COMPRAS:');
        try{
            Log::info('Acceso al Try-Catch de compras.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);
    ...
    {
        Log::info('LLegó al controller de data grupal para MERCO:');
        try{
            Log::info('Acceso al Try-Catch de merco.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);
    ...
    {
        Log::info('LLegó al controller de data grupal para INDICADORES:');
        try{
            Log::info('Acceso al Try-Catch de indicadores.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);
    ...
    {
        Log::info('LLegó al controller de data grupal para SOSTENIBILIDAD:');
        try{
            Log::info('Acceso al Try-Catch de sostenibilidad.');
            if (request()->ajax()) {
                $users_ids = explode(',', $userIds);
    ...
    public function agregar_evento_grupal_por_area(Request $request)
    {
        ...
        try {
            ...
            Log::info('agenda grupal de todo el area: '.json_encode($agendas_validate));
    ...
    public function lista_agendas_general(Request $request)
    {
        ...
        try{ // ADD
            if(isset($_SESSION['access_token']) && $_SESSION['access_token']){
                ...
                $id_areas = $areasUser->pluck('id')->toArray();
                Log::info('Areas obtenidas: '.json_encode($id_areas));
                $usuarios = DB::table('users')
                    ...
                    ->get();
                Log::info('Usuarios de las areas del user_log: '.json_encode($usuarios));
                ...
                log::info('Dato de tipo: '.json_encode($tipo));
                ...
                return view('oficial/admin_agendas_lider', [
                    ...
                    /*
                        'sesiones_redirec'  => $sesiones ?? null,
                        'data_paginacion'   => $data_paginacion,
                    */
                    'tipo'              => $tipo
                ]);
            }else{
    ...
    {
        Log::info('llegó al controller de todas las agendas');
        if(request()->ajax()){
            $espacio_data = Espacio::findOrFail($id);
            Log::info('Espacio encontrado en todas: '.json_encode($espacio_data));
            ...
            Log::info('Data obtenida de TODAS: '.json_encode($todas));

            $todas_por_pagina = $todasQuery->paginate(10);
            Log::info('Paginacion para todas: '.json_encode($todas_por_pagina));
    ...
    {
        Log::info('Llego al metodo de filtro agendadas por espacio');
        if(request()->ajax()){
            $espacio_data = Espacio::findOrFail($id);
            Log::info('Espacio encontrado en agendadas: '.json_encode($espacio_data));
            $espacio_id = $espacio_data->id;
            $usuario    = Auth::user();
            $idUserLog  = $usuario->id;
            ...
            Log::info('Data obtenida de agendadas: '.json_encode($agendadas));

            $agendadas_por_pagina = $agendadasQuery->paginate(10);
            Log::info('Paginacion para agendadas: '.json_encode($agendadas_por_pagina));
    ...
    public function filtrar_atendidas_agendas(int $id, Request $request)
    {
        if(request()->ajax()){
            ...
            Log::info('Data obtenida de atendidas: '.json_encode($atendidas));

            $atendidas_por_pagina = $atendidasQuery->paginate(10);
            Log::info('Paginacion para atendidas: '.json_encode($atendidas_por_pagina));
    ...
    public function filtrar_concluidas_agendas(int $id, Request $request)
    {
        if(request()->ajax()){
            $espacio_data = Espacio::findOrFail($id);
            Log::info('Espacio encontrado: '.json_encode($espacio_data));
            $espacio_id = $espacio_data->id;
            $usuario    = Auth::user();
            $idUserLog  = $usuario->id;
            ...
            Log::info('Data obtenida de concluidas: '.json_encode($concluidas));

            $concluidas_por_pagina = $concluidasQuery->paginate(10);
            Log::info('Paginacion para concluidas: '.json_encode($concluidas_por_pagina));
    ...
    {
        // Log::info('Llegó al controller de pendientes por agendar');
        // Log::info('ID DE ESPACIO: '.$id);
        if(request()->ajax()){
            $user_log       = Auth::user();
            ...
            // Log::info('TIPO_REU ENCONTRADA: '.$tipo_reunion_espacio);
            ...
            Log::info('data recibida para filtro de todas las agendas: ' . json_encode($log_message));
            $espacio            = '';
            $user_oto           = '';
            $espacio_grupal     = '';
            $usuarios_de_area   = '';
            if($tipo_reunion_espacio === 'individual'){
                ...
                Log::info('Espacios: '.json_encode($espacio));
                ...
                Log::info('Data de usuarios para OTO encontradas: '.json_encode($user_oto));
            }else{
                ...
                Log::info('Espacios con areas concatenadas encontradas: '.json_encode($espacio_grupal));
                ...
                Log::info('Data de usuarios grupales: '.json_encode($usuarios_de_area));
            }
    ...
    {
        Log::info('Llegó al controller de buscar usuarios por área.');
        try {
            $areaId = $request->input('area_id');
            $espacioId = $request->input('espacio_id');
            Log::info('Datos recibidos: '.json_encode('Area id: '.$areaId. ' Espacio id: '.$espacioId));
            $area = Area::find($areaId);
            $espacio = Espacio::find($espacioId);
            Log::info('Espacio encontrado: '.json_encode($espacio));
            $tipo_reunion_espacio = $espacio->tipo_reunion;
            Log::info('Tipo de reunion del espacio: '.json_encode($tipo_reunion_espacio));

            // Log::info('ID area: '.$areaId);
            // Log::info('espacio_nombre: '.$espacio);
            ...
            // Log::info('Usuarios encontrados para el espacio: '.json_encode($usuarios));
    ...
    {
        session_start();
        Log::info('Llegó al controller de la nueva vista para espacio_por_estado');
        if(isset($_SESSION['access_token']) && $_SESSION['access_token']){
            $this->client->setAccessToken($_SESSION['access_token']);
            $service = new Google_Service_Calendar($this->client);
            ...
            if ($request->has('sesiones')) {
                Log::info('si hay sesion');
                $sesiones = $request->input('sesiones'); // atendidas -> 1 ; pendientes -> 2
                session(['sesiones' => $sesiones]);
                Log::info('Parametro de sesiones recogida: '.json_encode($sesiones));
            }
    ...
    {
        Log::info('LLEG+O AL CONTROLLER DE AGENDAS_ATENDIDAS');
        if(request()->ajax()){
            $usuario    = Auth::user();
            $idUserLog  = $usuario->id;
    ...
    {
        Log::info('LLEGÓ AL CONTROLLER DE AGENDAS_PENDIENTES');
        if(request()->ajax()){
            $usuario_log = Auth::user();
            $id_user_log = $usuario_log->id;
            $username = $usuario_log->name;
            ...
            $data_de_usuarios = DB::table('users')
                ...
                ->where('espacios.id', function ($query) {
                    $query->select('id')
                        ->from('espacios')
                        ->where('tipo_reunion', 'individual');
                })
                ->where('users.id', '!=', $id_user_log) // Excluir al usuario actual
                ->groupBy('users.id', 'name', 'email')
                ->havingRaw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) >= (SELECT frecuencia FROM espacios WHERE tipo_reunion = "individual")')
                ->select('users.id as user_id', 'users.name as name', 'users.email as email', DB::raw('GROUP_CONCAT(areas.id) as area_id'), DB::raw('GROUP_CONCAT(areas.nombre) as area_name'), DB::raw('TIMESTAMPDIFF(DAY, MAX(agendas.fecha_hora_meet), NOW()) as ultimo_acceso'));

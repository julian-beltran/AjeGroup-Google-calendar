                        ...
                        <div class="col-lg-12 col-md-11 col-sm-11 second-content-form">
                            <form action="" class="p-2 rounded">
                                ...
                                <div class="row  text-center d-flex justify-content-center">
                                    <button type="submit" class="btn btn-outline-light btn-filter mr-2 col-3" id="buscar"><i class="fas fa-search mr-1"></i>Buscar</button>
...
@section('css')
    ...
    <style>
        ...
        .modal-header{
            color: #1F6C0D;
            background-color: #D4FFCA ;
            border-bottom: 1px solid #1F6C0D;
        }
        .btn-modal-cancel,
...
@section('js')
    ...
    <script type="text/javascript">
        $(function () {
            ...
            // Función para adjuntar eventos a elementos dinámicos
            function adjuntarEventos() {
                // Mostrar evidencia de la agenda
                $(document).on('click', '.show-evidencia', function (event){
                    event.preventDefault();
                    var url = $(this).data('url');
                    console.log('ID enviado:', url.split('/').pop());
                ...
                $('#sendEvidenciaAgenda').off('submit').on('submit', function (event){
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.agenda.anfitrion.subir_evidencia') }}",
                                type: 'POST',
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                },
                                ...
                                success:function (response){
                                    ...
                                    Swal.fire('¡Evidencia enviada!',
                                        'El envío de la evidencia ha sido correcto',
                                        'success'
                                    );
                                },
                                error:function (error){
                                    console.log('Error al enviar la evidencia', error);
                                    Swal.fire('¡Evidencia no enviado!',
                                        'Ocurrió un error al enviar la evidencia',
                                        'error'
                                    );
                                }
                            });
                        } else {
                            Swal.fire('¡Evidencia no enviada!',
                                'Se ha cancelado el envío de la evidencia',
                                'error'
                            );
                        }
                    });
                ...
                // Ver invitados de la agenda
                $(document).on('click', '.ver-invitados-agenda', function (event){
                    event.preventDefault();
                    var url = $(this).data('url');
                    console.log('ID enviado:', url.split('/').pop());
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function(data) {
                            $('#agendaInvitadosModal').modal('show');

                            // Establecer los datos de la agenda en los campos correspondientes
                            $('#id_agenda_invitado').text(data.agenda[0].agenda_id);
                            $('#fechaHoraMeet_agenda_invitado').text(data.agenda[0].fecha_hora_meet);

                            var tbody = $('.table-agenda-invitados');
                            tbody.empty(); // Limpiar el contenido anterior de la tabla

                            // Verificar si se devolvieron datos de la agenda
                            if (data.agenda.length > 0) {
                                // Iterar sobre los datos de la agenda
                                data.agenda.forEach(function(row) {
                                    var tr = $('<tr></tr>');
                                    tr.append('<td>' + row.id_agenda_invitado + '</td>');
                                    tr.append('<td>' + row.nombre_anfitrion + '</td>');
                                    tr.append('<td>' + row.nombre_invitado + '</td>');
                                    tr.append('<td>' + row.nombre_area + '</td>');
                                    tr.append('<td>' + row.nombre_espacio + '</td>');
                                    tbody.append(tr);
                                });
                            } else {
                                // Mostrar un mensaje si no hay datos de la agenda
                                tbody.append('<tr><td colspan="4">No se encontraron invitados para esta agenda.</td></tr>');
                            }
                        },
                        error: function(data) {
                ...
                // Agregar input para adjuntar imágenes
                $('#addFileInput').off('click').on('click', function () {
                    $('#fileInputsContainer').append('<input type="file" name="agenda_evidencia_file[]" id="agenda_evidencia_file" class="form-control mt-2"required>');
                });

                // Eliminar input de adjuntar imágenes
                $('.remove-file-button').off('click').on('click', function() {
                    $(this).closest('.input-group').remove();
                });

                // Limpiar el formulario de evidencia al cerrar el modal
                ...
                $('#buscar').on('click', function(event){
                    ...
                    table.ajax.url("{{ route('admin.agenda.anfitrion.lista') }}?desde=" + fecha_desde + "&hasta=" + fecha_hasta + "&espacio=" + espacio + "&area=" + area + "&estado=" +estado).load(null, 'reload');
                ...
                $('#reset').on('click', function(){
                    ...
                    table.ajax.url("{{ route('admin.agenda.anfitrion.lista') }}").load(null, 'reload');
            ...
            var table = $('.datatable_agendas').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.agenda.anfitrion.lista') }}",
                columns: [
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, row, meta){
                            ...
                            var fecha_hora_actual_formateada = fecha_hora_actual.toISOString().slice(0, 19).replace('T', ' ');
                            // Convertir fecha_hora_actual_formateada a formato YYYY-MM-DD HH:mm:ss
                            fecha_hora_actual_formateada = fecha_hora_actual_formateada.replace('T', ' ');
                            if (row.estado === 'terminado' || fecha_hora_actual_formateada <= row.fecha_hora) { // Si el estado es 'terminado' o fecha_actual es menor a la fecha meet, se desabilita el botoón de subir evidencias
                                links += ' <a href="javascript:void(0)" class="btn btn-outline-secondary show-evidencia mb-1" disabled><i class="fas fa-file-upload"></i></a>';
                            } else { // Si el estado no es 'terminado' y fecha hora actual es mayor a fecha meet, se habilita el botón para subir las evidencias
                                links += ' <a href="javascript:void(0)" data-url="/admin/agenda/anfitrion/ver_datos_agenda/' + row.agenda_id + '" class="btn btn-outline-secondary show-evidencia mb-1"><i class="fas fa-file-upload"></i></a>';
                            }*/
                            // links += ' <a href="javascript:void(0)" id="ver_evidencia" data-url="/admin/agenda/anfitrion/ver_evidencias/' + row.agenda_id + '"  class="btn btn-outline-primary download-evidencia-agenda"><i style="color: black;" class="fas fa-eye"></i></a>';
                            // links += ' <a href="javascript:void(0)" id="ver_evidencia" data-url="/admin/agenda/anfitrion/ver_invitados_agenda/' + row.agenda_id + '"  class="btn btn-outline-primary ver-invitados-agenda"><i style="color: green;" class="fas fa-user-friends"></i></a> ';
                            links += ' <a href="javascript:void(0)" id="ver_evidencia" data-url="/admin/agenda/anfitrion/edit_agenda/' + row.agenda_id + '"  class="btn btn-outline-primary editar-agenda"><i style="color: orange;" class="fas fa-edit"></i></a> '; // ------------
                            links += ' <a href="javascript:void(0)" id="eliminar_evidencia" data-url="/admin/agenda/anfitrion/delete_agenda/' + row.agenda_id + '"  class="btn btn-outline-primary eliminar-agenda"><i style="color: red;" class="fas fa-times-circle"></i></a> '; // ------------
            ...
            adjuntarEventos(); // Adjuntar eventos al cargar la página

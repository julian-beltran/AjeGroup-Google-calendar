...
@section('content')
                ...
                <div class="p-2 w-100">
                    <div class="row justify-content-center align-items-center mb-2">
                        <div class="col-lg-12 col-md-11 col-sm-11">
                            <form action="" class="p-4 rounded"> <!--bg-light-->
                                ...
                                <div class="row text-center d-flex">
                                    <div class="col-md-6 mb-3">
                                        <button type="button" class="btn btn-outline-success mr-2 col-3" id="buscar" onclick="filtrarUsuarios()"><i class="fas fa-search mr-1"></i>Buscar</button>
                                        <button type="button" class="btn btn-secondary col-3" id="reset" onclick="restablecerFiltro()"><i class="fas fa-trash-alt mr-1"></i>Limpiar</button>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <button type="button" class="btn btn-outline-success btn-modal-cancel button-evidencia col-3 button-cancel-user float-end d-flex justify-content-center align-items-center align-content-center" data-bs-toggle="modal" data-bs-target="#usuarioAddExcelModal">Subir excel <i class='bx bxs-file-import'></i></button>
                                    </div>
                                </div>
                            </form>
                        </div>
    ...
    <div class="modal fade" id="usuarioAddModal" tabindex="-1" aria-labelledby="usuarioAddModalLabel" aria-hidden="true">
    ...
    <div class="modal fade" id="usuarioAdminModal" tabindex="-1" aria-labelledby="usuarioAdminModalLabel" aria-hidden="true">
    ...
    <div class="modal fade" id="updateUsuario" tabindex="-1" aria-labelledby="updateUsuarioModal" aria-hidden="true">
        <div class="modal-dialog modal-md">
    ...
    {{-- Begin send users for excel --}}
    <div class="modal fade" id="usuarioAddExcelModal" tabindex="-1" aria-labelledby="usuarioAddExcelModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <form id="formAddExcelUsuario" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="usuarioAddLabel">Subir archivo excel</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <input type="file" class="form-control-file w-100" name="excelFile" id="excelFile" accept=".xlsx, .xls">
                    </div>
                    <div class="modal-footer d-block">
                        <button type="submit" class="btn btn-outline-success button-evidencia button-add-user mb-2">Subir archivo <i class='bx bxs-file-import' style="font-size: 20px; font-weight: bold;"></i></button>
                        <button type="button" class="btn btn-outline-light btn-modal-cancel button-evidencia button-cancel-user" data-bs-dismiss="modal">Cerrar <i class="fas fa-times ml-2" style="font-size: 20px; font-weight: bold;" ></i></button>
                    </div>
                </div>
            </form>
    ...
    {{-- End modal send users for excel --}}
@stop
...
@section('js')
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('.datatable_usuarios_admin').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d){
                        d.nombre  = $('#nameInput').val();
                        d.email   = $('#emailInput').val();
                    }
                },
                columns: [
                    {data: 'id', name: 'id'},
            ...
            /**********************************************************************************************/
            $('#formAddExcelUsuario').submit(function(event) {
                event.preventDefault();

                Swal.fire({
                    title: '¿Está seguro de enviar el archivo excel?',
                    icon: 'info',
                    showCancelButton: true,
                    confirmButtonColor: '#20c997',
                    cancelButtonColor: '#6c757d',
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire('¡Excel no enviado!',
                            'Se ha cancelado el envío del excel',
                            'error'
                        );
                    }
                });
        ...
        function asignarAdministracion(id_usuario){ // Administracion del usuario
            $.ajax({
                url: '/admin/usuario/settings/' + id_usuario,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                dataType: "json",
        ...
        function modificarUsuario(id_usuario){ // Modificar datos del usuario
            $.ajax({
                url: '/admin/usuario/edit/' + id_usuario,
                type: "GET",
                dataType: "json",
                success: function(data){
        ...
        function eliminarUsuario(event, id_usuario){ // Eliminar el usuario
            event.preventDefault();
            ...
            Swal.fire({
                ...
                if(result.isConfirmed){
                    ...
                    $.ajax({
                        url: '/admin/usuario/usuario_delete/' + id_usuario,
                        type: "DELETE",
                        dataType: "json",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
